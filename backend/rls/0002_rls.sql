...
-- * User-owned tables filter on `user_id = auth.uid()`.
-- * profiles filters on `id = auth.uid()`.
...
-- Every policy is keyed so Supabase Auth injects `auth.uid()` per request.
-- =============================================================================
...
create policy programs_crud on public.programs
  ...
  using (exists (
    ...
    where j.id = programs.journey_id and j.user_id = auth.uid()))
  with check (exists (
    select 1 from public.journeys j
    where j.id = programs.journey_id and j.user_id = auth.uid()));
...
create policy lessons_crud on public.lessons
  ...
  using (exists (
    ...
    where p.id = lessons.program_id and j.user_id = auth.uid()))
  with check (exists (
    select 1 from public.programs p
    join public.journeys j on j.id = p.journey_id
    where p.id = lessons.program_id and j.user_id = auth.uid()));
...
create policy notifications_crud on public.notifications
  for all using (user_id = auth.uid()) with check (user_id = auth.uid());
-- Convenience: mark-as-read only touches `read`/`updated_at` (defence in depth).
